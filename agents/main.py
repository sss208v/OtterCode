...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="otter-code",
        description="Otter Code — a minimal coding agent",
        add_help=False,
    )
...
def _resolve_api_config(cli_api_base: str | None) -> tuple[str | None, str | None, bool]:
    generic_api_key = _clean_env(os.environ.get("APIKEY")) or _clean_env(os.environ.get("OTTER_CODE_API_KEY"))
    openai_api_key = _clean_env(os.environ.get("OPENAI_API_KEY"))
    anthropic_api_key = _clean_env(os.environ.get("ANTHROPIC_API_KEY"))

    generic_api_base = _clean_env(os.environ.get("API")) or _clean_env(os.environ.get("OTTER_CODE_API_BASE"))
...
async def run_repl(agent: Agent) -> None:
    ...
    while True:
        ...
        if inp == "/skills":
            ...
            if not skills:
                print_info("No skills found. Add skills to .otter/skills/<name>/SKILL.md")
            else:
...
def main() -> None:
    ...
    if args.help:
        # 自定义帮助文本，展示 Otter Code 支持的启动参数和 REPL 内置命令。
        print("""
Usage: otter-code [options] [prompt]
...
Examples:
  otter-code "fix the bug in src/app.ts"
  otter-code --yolo "run all tests and fix failures"
  otter-code --plan "how would you refactor this?"
  otter-code --max-cost 0.50 --max-turns 20 "implement feature X"
  MODEL=deepseek-chat APIKEY=sk-xxx API=https://api.deepseek.com/anthropic otter-code "hello"
  MODEL=gpt-4o OPENAI_API_KEY=sk-xxx OPENAI_BASE_URL=https://aihubmix.com/v1 otter-code "hello"
  otter-code --resume
  otter-code  # starts interactive REPL
""")
        sys.exit(0)
