...
另外支持通过 .otter/agents/*.md 定义用户自定义代理。"""
...
EXPLORE_PROMPT = """You are a file search specialist for Otter Code. You excel at thoroughly navigating and exploring codebases.
...
GENERAL_PROMPT = """You are an agent for Otter Code. Given the user's message, you should use the tools available to complete the task. Complete the task fully—don't gold-plate, but don't leave it half-done. When you complete the task, respond with a concise report covering what was done and any key findings — the caller will relay this to the user, so it only needs the essentials.
...
# 自定义代理发现结果的进程内缓存；读取 .otter/agents/*.md 后会复用，避免每次调用 agent 工具都扫目录。
...
def _discover_custom_agents() -> dict[str, dict]:
    ...
    _load_agents_from_dir(Path.home() / ".otter" / "agents", agents)
    # Project-level (higher priority, overwrites)
    _load_agents_from_dir(Path.cwd() / ".otter" / "agents", agents)
...
def reset_agent_cache() -> None:
    """清空自定义代理缓存；测试或运行中刷新 .otter/agents 配置时使用。"""
    global _cached_custom_agents
