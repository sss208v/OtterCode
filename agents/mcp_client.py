...
配置来源：
- 全局配置：`~/.otter/settings.json`
- 项目配置：`.otter/settings.json`
...
class McpConnection:
    ...
    async def connect(self) -> None:
        ...
        # Windows 下 npx 等命令实际是 npx.cmd，create_subprocess_exec 不做 PATHEXT
        # 解析，直接传命令名会报 WinError 2；用 shutil.which 解析出完整路径。
        resolved_command = shutil.which(self.command)
        if not resolved_command:
            raise FileNotFoundError(
                f"MCP command '{self.command}' not found in PATH. "
                f"Check that it is installed (e.g. Node.js for npx-based servers)."
            )
    ...
    async def initialize(self) -> None:
        ...
        await self._send_request("initialize", {
            ...
            "capabilities": {},
            "clientInfo": {"name": "otter-code", "version": "1.0.0"},
        })
...
class McpManager:
    ...
    def _load_configs(self) -> dict[str, dict]:
        ...
        # 1. 全局配置：~/.otter/settings.json
        global_path = Path.home() / ".otter" / "settings.json"
        self._merge_config_file(global_path, merged)

        # 2. 当前项目配置：<cwd>/.otter/settings.json
        project_path = Path.cwd() / ".otter" / "settings.json"
