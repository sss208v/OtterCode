"""Online skill evaluation — layered eval system for skill evolution quality.

设计来源（2026 评测领域进展）：
- 零 token 层：programmatic rules（Anthropic "Demystifying evals" 的确定性 grader 优先原则）
- LLM judge 层：flash 模型做二元判断，prompt 带"不确定即判负"（自偏好偏差研究的安全侧设计）
- 配对评测层：SkillsBench 方法论，有 skill / 无 skill 两组样本对比 pass rate delta
- 对抗式 critic：DEBATE 简化版，晋升路径追加找茬审查
- 预算控制：VeRO 的 budget-controlled evaluation，LLM 调用超限自动降级
- 时间切分：mutate_dev（较早 75%）/ promotion_test（较晚 25%），验证演化能否泛化到未来
"""
...
class _BudgetExhausted(Exception):
    # 内部信号：LLM judge 调用预算耗尽，调用方降级为纯 programmatic 评测。
    pass
...
async def _evaluate_rule_async(
    ...
    *,
    ...
    system = (
        ...
        'Schema: {"reason": "short reason", "pass": true|false}\n'
        "First write a short reason for your verdict, then the verdict. "
        "Judge only against the requirement provided. "
        "If you are not clearly confident, return pass: false. "
        "Prefer false when the requirement is not clearly satisfied. "
    ...
    try:
        ...
        return {
            ...
            "passed": False,
            "skipped": True,
            "details": {"reason": "judge_budget_exhausted"},
        }
    except Exception as exc:
    ...
    *,
    ...
    def add_sample(source: dict[str, Any], source_kind: str) -> None:
        ...
        if not messages or not _latest_message(messages, "user"):
            return
        # 配对评测：检索到 skill 上下文的会话归"有 skill"组，其余为"无 skill"基线组。
        has_skill_context = bool(source.get("retrieved_reference"))
...
def _assign_replay_splits(samples: list[dict[str, Any]]) -> list[dict[str, Any]]:
    # 时间切分（SkillsBench 配对 + 未来泛化验证）：较早样本做 mutate_dev，较晚样本做 promotion_test。
    out = [dict(item) for item in samples]
    if len(out) < 2:
        for item in out:
            item["split"] = "mutate_dev"
        return out
    ordered_by_time = sorted(out, key=lambda item: (str(item.get("time") or ""), item["sample_id"]))
    split_at = max(1, int(round(len(ordered_by_time) * DEFAULT_DEV_SPLIT_RATIO)))
    for idx, item in enumerate(ordered_by_time):
        item["split"] = "mutate_dev" if idx < split_at else "promotion_test"
...
def _paired_metric(replay_pool: list[dict[str, Any]], rule_summary: dict[str, Any]) -> dict[str, Any]:
    # SkillsBench 配对评测：有 skill 组 vs 无 skill 组的 pass rate 差。
    sample_ctx = {str(s.get("sample_id") or ""): bool(s.get("has_skill_context")) for s in replay_pool}
    with_outcomes = [o for o in rule_summary.get("outcomes", []) if sample_ctx.get(str(o.get("sample_id") or ""))]
    without_outcomes = [o for o in rule_summary.get("outcomes", []) if not sample_ctx.get(str(o.get("sample_id") or ""))]
    with_rate = _ratio(sum(1 for o in with_outcomes if o.get("passed")), len(with_outcomes))
    without_rate = _ratio(sum(1 for o in without_outcomes if o.get("passed")), len(without_outcomes))
    return {
        "with_skill_pass_rate": with_rate,
        "without_skill_pass_rate": without_rate,
        "delta": round(with_rate - without_rate, 4),
        "with_skill_samples": len(set(str(o.get("sample_id") or "") for o in with_outcomes)),
        "without_skill_samples": len(set(str(o.get("sample_id") or "") for o in without_outcomes)),
    }
...
def _skill_status(
    *,
    ...
    # 配对评测门控：有 skill 组与无 skill 基线组都存在样本且无增益时阻止晋升。
    if int(paired.get("with_skill_samples", 0) or 0) > 0 and int(paired.get("without_skill_samples", 0) or 0) > 0:
        if float(paired.get("delta", 0.0) or 0.0) <= 0:
            reasons.append(f"no skill gain vs baseline (paired delta {_pct(float(paired.get('delta', 0.0) or 0.0))})")
...
async def _run_critic_async(
    *,
    ...
    # DEBATE 简化版：晋升关键路径的对抗审查，发现 judge 判断过宽或遗漏的违规。
    if side_query is None:
        return {"passed": True, "skipped": True, "details": {"reason": "critic_llm_not_configured"}}
    system = (
        "You are a strict critic for a skill promotion review.\n"
        "Output ONLY strict JSON parseable by json.loads.\n"
        'Schema: {"pass": true|false, "reason": "short reason"}\n'
        "Review the candidate's reported failures and the skill being promoted. "
        "Veto (pass: false) if the judge was too lenient, a reported failure is more serious than judged, "
        "or the skill's instructions conflict with observed failures. "
        "If the evaluation looks sound and the candidate has no serious unaddressed issues, approve (pass: true).\n"
    )
    payload = {
        "skill_name": skill_name,
        "candidate_score": float(candidate.get("average_score", 0.0) or 0.0),
        "hard_failures": int(candidate.get("hard_failures", 0) or 0),
        "failures": list(failures or [])[:6],
    }
    try:
        parsed = _parse_json_object(await side_query(system, json.dumps(payload, ensure_ascii=False)))
    except _BudgetExhausted:
        return {"passed": True, "skipped": True, "details": {"reason": "critic_budget_exhausted"}}
    except Exception as exc:
        return {"passed": True, "skipped": True, "details": {"reason": f"critic failed: {exc}"}}
    reason = str(parsed.get("reason") or "").strip() or "critic returned no reason"
    return {
        "passed": bool(parsed.get("pass", False)),
        "skipped": False,
        "details": {"reason": reason[:500]},
...
async def _persist_eval_artifacts(
    *,
    skill_name: str,
    snapshot: dict[str, Any],
    replay_pool: list[dict[str, Any]],
    ...
    # 落盘评测产物 + champion 晋升判定 + 对抗式 critic 审查（晋升关键路径）。
    lineage_id = _lineage_id_for_skill(skill_name)
    ...
    outputs = [
        {
            ...
            "has_skill_context": sample.get("has_skill_context", False),
            "response_source": "history_latest_assistant",
    ...
    min_score_delta = float(os.environ.get("OTTER_EVAL_MIN_SCORE_DELTA", DEFAULT_MIN_SCORE_DELTA) or DEFAULT_MIN_SCORE_DELTA)
    ...
    critic: dict[str, Any] = {"passed": True, "skipped": True, "details": {"reason": "critic_not_required"}}
    if promotion.get("promoted"):
        # 晋升关键路径：对抗式 critic 审查 judge 是否过宽。
        critic = await _run_critic_async(
            skill_name=skill_name,
            candidate=candidate,
            failures=rule_summary.get("failures", []),
            side_query=side_query,
        )
        promotion["critic"] = critic
        judgments.append(
            {
                "sample_id": "",
                "variant_id": "current_active",
                "split": "promotion_test",
                "rule_id": "promotion_critic",
                "label": "Promotion critic review",
                "kind": "critic",
                "hard": True,
                "passed": bool(critic.get("passed")),
                "skipped": bool(critic.get("skipped")),
                "score": 0.0,
        ...
        if not critic.get("passed"):
            promotion = {
                "promoted": False,
                "status": "rejected",
                "reason": "critic vetoed promotion: " + str((critic.get("details") or {}).get("reason") or ""),
...
async def _evaluate_online_skill_evolution_core(
    *,
    ...
    # VeRO 预算控制：LLM judge 调用超限后降级为纯 programmatic 评测。
    budget = {"used": 0, "exhausted": False}

    async def budgeted_side_query(system: str, user_message: str) -> str:
        if side_query is None:
            return ""
        if budget["used"] >= max_judge_calls:
            budget["exhausted"] = True
            raise _BudgetExhausted()
        budget["used"] += 1
        return await side_query(system, user_message)

    bq: SideQuery | None = budgeted_side_query if side_query is not None else None
    ...
    # 单 lineage 增量评测：指定 skill_name 时只评测该 skill（事件驱动场景，控制 token 成本）。
    if skill_name:
        all_names = {str(skill_name).strip()} if str(skill_name).strip() in all_names else set()
        if not all_names:
            return {"generated_at": _utc_now(), "mode": "online_skill_lineage_eval", "data_dir": str(root), "skills": [], "aggregate": {}, "recent_failures": [], "note": f"skill not found: {skill_name}"}
    ...
    for name in sorted(all_names):
        ...
        paired = _paired_metric(replay_pool, rule_summary)
        ...
        current_version = lineage.get("current_version", lifecycle.get("version", ""))
        snapshot["version"] = str(current_version or "")
        artifacts = (
            await _persist_eval_artifacts(
                skill_name=name,
                snapshot=snapshot,
                replay_pool=replay_pool,
                rules=rules,
        ...
        # 配对 delta 持久化到 provenance index，供 /journey 展示基线对比。
        if isinstance(provenance_index, dict):
            item = provenance_index.setdefault(name, {})
            if isinstance(item, dict):
                item["last_paired_delta"] = paired.get("delta", 0.0)
                item["last_paired_with"] = paired.get("with_skill_pass_rate", 0.0)
                item["last_paired_without"] = paired.get("without_skill_pass_rate", 0.0)
                # 无 skill 基线组存在样本时填充创建时的占位（SkillsBench 配对基线）。
                if int(paired.get("without_skill_samples", 0) or 0) > 0:
                    item.setdefault("no_skill_baseline", paired.get("without_skill_pass_rate", 0.0))
    ...
    if isinstance(provenance_index, dict) and provenance_index:
        _write_json(root / ONLINE_PROVENANCE_INDEX, provenance_index)
    ...
    report = {
        ...
        "methodology": {
            ...
            "paired": "SkillsBench paired evaluation: compare with-skill vs without-skill baseline pass rates",
            "split": "time-based split: earlier samples mutate_dev, later samples promotion_test",
            "gate": "mark skills incubating, watch, healthy, pruned, or unobserved from replay, rule, paired, and usage signals",
            "champion": "promote the current active version into a local online-eval champion only when healthy, beats the prior champion gate, and passes the adversarial critic",
        },
        "llm_judge": {
            "enabled": bool(side_query),
            "rule_kind": "llm_binary",
            "response_source": "history_latest_assistant",
            "budget": {"max_judge_calls": max_judge_calls, "used": budget["used"], "exhausted": budget["exhausted"]},
        },
        "thresholds": {
            "min_replay_samples": min_replay_samples,
            "min_promotion_tests": min_promotion_tests,
    ...
    *,
    ...
    return asyncio.run(
        _evaluate_online_skill_evolution_core(
            ...
            min_relevance_rate=min_relevance_rate,
            min_rule_pass_rate=min_rule_pass_rate,
            max_judge_calls=int(os.environ.get("OTTER_EVAL_MAX_JUDGE_CALLS", DEFAULT_MAX_JUDGE_CALLS) or DEFAULT_MAX_JUDGE_CALLS),
...
def format_online_skill_eval(report: dict[str, Any] | None = None) -> str:
    ...
    budget = llm_judge.get("budget") if isinstance(llm_judge.get("budget"), dict) else {}
    ...
    lines = [
        ...
        (
            ...
            f"judge_calls={budget.get('used', 0)}/{budget.get('max_judge_calls', '?')}"
        ),
        (
            "  actions: "
    ...
    if budget.get("exhausted"):
        lines.append("  budget_exhausted=true (LLM rules downgraded to programmatic-only)")
    if statuses:
    ...
    if not skills:
        ...
        for item in ranked[:20]:
            ...
            if int(paired.get("with_skill_samples", 0) or 0) > 0 and int(paired.get("without_skill_samples", 0) or 0) > 0:
                paired_str = (
                    f", paired_delta={_pct(float(paired.get('delta', 0.0) or 0.0))}"
                    f"(with={_pct(float(paired.get('with_skill_pass_rate', 0.0) or 0.0))}"
                    f"/without={_pct(float(paired.get('without_skill_pass_rate', 0.0) or 0.0))})"
                )
            lines.append(
                "    "
